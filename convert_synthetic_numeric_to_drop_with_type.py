...
from create_examples_n_features_with_type import read_file, write_file
...
def convert_synthetic_texual_to_drop(data):
    ...
    for d in data:
        _id, expr, val,intermed_values = d['id'], d['expr'].strip(), str(d['val']).strip(),d['intermed_values']
        data_type = d['type']
        sample = {'passage': expr,'type':data_type,'intermed_values':intermed_values}
        ...
        try:
            ...
            is_num = False
            assert val in expr
        # a = {'number': '', 'date': {'day': '', 'month': '', 'year': ''}, 'spans': []} #numberしか使わんのに
        a = {'number':'','spans':[]}
        if is_num:
            a['number'] = val
        else:
...
def main():
    ...
    # assert train_nums.isdisjoint(dev_nums) #別に答えの値が被ってもよくない？？？？・
